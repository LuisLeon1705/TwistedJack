function dealCard(hand, location) {
    ...
    var currentTotal = (currentTurn === "player") ? playerHandTotal : 
                       (currentTurn === "playerSplit" ? playerSplitHandTotal : dealerHandTotal);
    ...
    switch(hand[index].name) {
        ...
        case "exponential":
            ...
            updateTotal((hand[index].value === 2) ? Math.pow(currentTotal, 2) : Math.pow(currentTotal, 3));
            break;
        case "root":
            // Si value es 2 se toma raíz cuadrada; si es 3, raíz cúbica.
            updateTotal((hand[index].value === 2) ? Math.sqrt(currentTotal) : Math.cbrt(currentTotal));
            break;
        case "trigo":
...
function isPlayerDone() {
    ...
    if (splitGame === false && (playerHandTotal >= 21 || playerHandTotal <= -10)) {
        gameOver();
...
function reduceAcesValue(deck) {
    for (var i = 0; i < deck.length; i++) {  
        if (deck[i].name === "ace" && deck[i].value === 11) { // Solo se revisan ases que no han sido cambiados de 11 a 1
            deck[i].value = 1;
            if (currentTurn === "player") {
                playerHandTotal -= 10;
            } else if (currentTurn === "playerSplit") {
