...
function updateVisibleHandTotals() {
	$(playerHandTotalDisplay).text(playerHandTotal.toFixed(2));
	$(playerSplitHandTotalDisplay).text(playerSplitHandTotal);

	if (dealerHand.length === 2 && isGameOver === false && dealerStatus === "start") {
		$(".dealer-hand-total").text((dealerHandTotal - dealerHand[1].value).toFixed(2));
	} else {
		$(".dealer-hand-total").text((dealerHandTotal).toFixed(2));
	}
