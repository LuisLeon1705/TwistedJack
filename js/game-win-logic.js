...
function gameOver() {
	...
	if (dealerHandTotal === 21) {
	...
	} else if (dealerHandTotal > 21 || dealerHandTotal < -5) {
		if (playerHandTotal <= 21 && playerHandTotal > -5) {
			gameWinner = "player";
		} else if (splitGame === true && playerSplitHandTotal <= 21 && playerSplitHandTotal > -5) {
			gameWinner = "player";
		} else {
			gameWinner = "tie";
		}
		...
		if (playerHandTotal === 21  || playerSplitHandTotal === 21) {
			...
			gameWinner = "tie";
		}else if(playerHandTotal > -5 && dealerHandTotal < 21 && dealerHandTotal > -5){
			gameWinner = "dealer";
		}else {
			gameWinner = "dealer";
		}
...
function announceWinner() {
	...
	if (gameWinner === "player") {
		...
		$("#game-outcome").text("¡Gana el crupier!");
	} else if (gameWinner === "tie") {
		$("#game-outcome").text("¡Empate!");
	}
}