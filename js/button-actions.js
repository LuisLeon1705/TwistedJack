...
var startGame = function() {
	getCards();
	if (currentWager === 0) {
		Materialize.toast("Para jugar tienes que escoger una apuesta", 1000);
	} else if (currentChipBalance < 10) {
		Materialize.toast("¡Se te acabaron las fichas! Reinicia el juego para recuperarlas" , 2000);
	} else if (currentChipBalance < currentWager) {
		Materialize.toast("Fichas insuficientes, escoja una apuesta menor" , 1500);
	} else {
		currentChipBalance -= currentWager;
		updateVisibleChipBalances();
		$("#welcome").hide();
		$("#game-over").hide();
		$(".brand-logo").text("TwistedJack"); 
...
function doubleDown() {
	if (currentChipBalance - currentWager <= 0) {
		Materialize.toast("Fichas insuficientes" , 1000);
	}
	else {
